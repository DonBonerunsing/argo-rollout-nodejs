pipeline {
    ...
    environment {
    DOCKERHUB_CREDENTIALS = credentials('danny-dockerhub2')
    APP_NAME = "danny1dockerhub/argo-rollout-bluegreen"
    }
    stages { 
        stage('SCM Checkout') {
            steps{
           git branch: 'main', url: 'https://github.com/DonBonerunsing/Argo-rollout-nodejs.git'
            }
        }
        // run sonarqube test
        stage('Run Sonarqube') {
